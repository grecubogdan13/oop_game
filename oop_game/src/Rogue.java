...
public class Rogue extends Character {
    public Rogue(String name, int level, int experience){
        ...
        if(level%2==0) {
            ...
            setStrength(level/2);
            setCharisma(level/2+1);
        }
        setCharacterInventory(15,50);
        setCurrentLife(200);
        setMaxLife(200);
        setMaxMana(100);
        setCurrentMana(100);
    ...
    public int getDamage(){
        ...
        if (miss > 50)
            miss = 50;
        if(chance>miss){
            System.out.println("You attacked your enemy for "+(10+2*getDexterity())+" damage.");
            return 10+2*getDexterity();
        }
        else {
            System.out.println("You attacked your enemy for "+(20+4*getDexterity())+" damage.");
            return 20+4*getDexterity();
        }
    }
}
